...
namespace mu2eii {
  ...
  constants::constants(int Mode) : constants() {
    const float eff_ur_1b     (0.994);
    const float eff_ur_2b     (0.986);
    const float deadtime_1b   (1.67927e-02);
    const float deadtime_2b   (7.62028e-02);
    const float lumi_cutoff_1b(1-0.99342);
    const float lumi_cutoff_2b(1-0.88785);
    const float trig_eff      (0.98);

    int mu2eIIMode = (Mode % 100) / 10;
    Mode %= 10;
