...
// m3->scan_pmin(104.0,105.0,700.,1700)
//
...
//-----------------------------------------------------------------------------
  ...
  else if (Mode == 2) {                 // emulate 30% degradation of the light yield
     fSFSignal  = 1.;
     fSFCosmics = 0.21/0.046;
  }
  else if (Mode == 3) {                 // scale proton flux to take into account the dead time
                                        // and the proton pulse intensity cut-off by the simulation
                                        // assume above 12e8 efficiency goes down linearly, and 12% statistics add (6+/-6)%
                                        // luminosity cutoff fractions are printed by su2020/analysis/proton_pulse_intensity.C

    npot_1b    = npot_1b*(1-deadtime_1b)*eff_ur_1b*(1-lumi_cutoff_1b/2.)*mu2eii_beam;
    npot_2b    = npot_2b*(1-deadtime_2b)*eff_ur_2b*(1-lumi_cutoff_2b/2.)*mu2eii_beam;
    fSFSignal  = 1.00*trig_eff;
    fSFCosmics = 1.00*trig_eff*mu2eii_time;
  }
  else if (Mode == 4) {                 // scale proton flux to take into account the dead time
                                        // and the proton pulse intensity cut-off by the simulation
                                        // assume above 12e8 efficiency goes down linearly, and 12% statistics add (6+/-6)%
                                        // luminosity cutoff fractions are printed by su2020/analysis/proton_pulse_intensity.C

    npot_1b    = npot_1b*(1-deadtime_1b)*eff_ur_1b*(1-lumi_cutoff_1b/2.);
    npot_2b    = npot_2b*(1-deadtime_2b)*eff_ur_2b*(1-lumi_cutoff_2b/2.);
    fSFSignal  = 1.00*trig_eff;
    fSFCosmics = 0.21/0.046*trig_eff;
  }
...
//-----------------------------------------------------------------------------
  ...
  float Rmue = 1.e-15;
  mu2eii::channel* ce = new mu2eii::channel("CE",npot_1b,npot_2b,fSFSignal);
  SetSignal(ce,Rmue);
//-----------------------------------------------------------------------------
// 2. cosmics: fit , NPOT numbers are not used
//-----------------------------------------------------------------------------
  mu2eii::channel* cosmics = new mu2eii::channel("Cosmics",npot_1b,npot_2b,fSFCosmics);
  AddBgrChannel(cosmics);
//-----------------------------------------------------------------------------
// 3. DIO
//-----------------------------------------------------------------------------
  mu2eii::channel* dio = new mu2eii::channel("DIO",npot_1b,npot_2b,fSFSignal);
  AddBgrChannel(dio);
//-----------------------------------------------------------------------------
// 4. pbars
//-----------------------------------------------------------------------------
  mu2eii::channel* pbar = new mu2eii::channel("PbarTOT",npot_1b,npot_2b,fSFSignal*(mu2eii_beam == 1.));
  AddBgrChannel(pbar);
...
//-----------------------------------------------------------------------------
  ...
  if (rpc_config_code == 0)  rpc = new mu2eii::channel_rpc("RPC",npot_1b,npot_2b,fSFSignal,rpc_config_code);
  if (rpc_config_code == 1)  rpc = new mu2eii::channel_rpc("RPC",npot_1b,npot_2b,fSFSignal,rpc_config_code);
...
//-----------------------------------------------------------------------------
  printf("---------------------------------------------------------------\n");
   ...
   rpc_oot = new mu2eii::channel_rpc("RPC_OOT",npot_1b,npot_2b,fSFSignal,rpc_config_code);
  return 0;
}
