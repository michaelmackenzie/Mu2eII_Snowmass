...
bool comparePDF_ = true; //make a plot comparing full random sample to semi-random sampling
...
double scalePrecision_ = 1.; //increase or decrease numerical precision
int verbose_ = 1;
bool printSpectators_ = true;
...
void Mu2e_model() {
  ...
  const double dio_bkg          = (useLYSafetyFactor_) ? 6.55531e-2 : 3.77436e-2;
  const double dio_frac_unc     = scaleUncertainty_*0.70; //using systematic upper estimate with statistical error
  const double rpc_bkg          = (useLYSafetyFactor_) ? 3.85590e-2 : 1.057920e-2;
  ...
  const double pbar_bkg         = (useLYSafetyFactor_) ? 1.15306e-2 : 1.01270e-2;
  const double pbar_frac_unc    = scaleUncertainty_*1.; //100% uncertainty quoted - careful with Gaussian mode!

  const double cr_lo_bkg        = (useLYSafetyFactor_) ? 2.43785e-1 : 4.74529e-2;
  ...
  const double one_batch_pot = 2.8631579e19; //one-batch mode POT, for separate batch uncertainties
  const double two_batch_pot = 9.0285712e18; //two-batch mode POT, for separate batch uncertainties
  const double tot_pot = one_batch_pot + two_batch_pot;

  const double signal_acceptance = (useLYSafetyFactor_) ? 0.13178 : 0.11735881;
  ...
  const double ses = 1./(scaleLuminosity_*tot_pot*1.59e-3*0.609); //for signal strength -> R_mue, SES if acceptance was 100%
